...
public class ProfileItem
{
    ...
    public List<string>? GetAlpnForTransport()
    {
        var alpn = GetAlpn();
        if (GetNetwork() != nameof(ETransport.grpc))
        {
            return alpn;
        }
        ...
        var h2Index = normalized.FindIndex(x => x.Equals("h2", StringComparison.OrdinalIgnoreCase));
        if (h2Index > 0)
        {
            normalized.RemoveAt(h2Index);
            normalized.Insert(0, "h2");
        }
        else if (h2Index < 0)
        {
            normalized.Insert(0, "h2");
