...
public partial class CoreConfigSingboxService
{
    ...
    private void GenDnsServers()
    {
        ...
        NormalizeProxyRemoteDns(remoteDns);
    ...
    {
        var finalDns = ShouldUseSystemLocalDns()
            ? new Server4Sbox { type = "local" }
            : ParseDnsAddress(context.SimpleDnsItem?.BootstrapDNS ?? Global.DomainPureIPDNSAddress.First());
    ...
    private void GenDnsRules()
    {
        ...
        if (context.ProtectDomainList.Count > 0)
        {
            _coreConfig.dns.rules.Add(new()
            {
                // Resolve the proxy server domain via the system resolver in TUN+gRPC mode.
                server = ShouldUseSystemLocalDns() ? Global.SingboxLocalDNSTag : Global.SingboxDirectDNSTag,
    ...
    {
        if (!ShouldUseSystemLocalDns())
        {
            return;
        }
        if (!string.Equals(remoteDns.detour, Global.ProxyTag, StringComparison.Ordinal))
        {
            return;
        }
        if (!string.Equals(remoteDns.type, "udp", StringComparison.OrdinalIgnoreCase))
        {
            return;
        }

        remoteDns.type = "tcp";
        remoteDns.server_port ??= 53;
    }
