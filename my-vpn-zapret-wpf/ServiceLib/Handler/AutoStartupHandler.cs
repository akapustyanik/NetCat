...
public static class AutoStartupHandler
{
    ...
    private static async Task SetTaskWindows(Config config)
    {
        try
        {
            var autoRunName = GetAutoRunNameWindows();
            var exePath = Utils.GetExePath();
            var arguments = GetAutoRunArgumentsWindows(config);
            if (Utils.IsAdministrator())
            {
                AutoStartTaskService(autoRunName, exePath, arguments, "");
            }
            else
            {
                WindowsUtils.RegWriteValue(Global.AutoRunRegPath, autoRunName, BuildAutoRunCommandWindows(exePath, arguments));
            }
        }
        catch (Exception ex)
        {
    ...
    public static void AutoStartTaskService(string taskName, string fileName, string arguments, string description)
    {
        ...
        task.Actions.Add(new Microsoft.Win32.TaskScheduler.ExecAction(fileName, arguments, Path.GetDirectoryName(fileName)));

        taskService.RootFolder.RegisterTaskDefinition(taskName, task);
    }

    private static string BuildAutoRunCommandWindows(string exePath, string arguments)
    {
        var command = exePath.AppendQuotes();
    ...
    private static string GetAutoRunArgumentsWindows(Config config)
    {
        var arguments = new List<string>();
        if (ShouldStartMinimizedToTray(config))
        {
            arguments.Add(Global.StartMinimizedToTray);
        }

        return string.Join(' ', arguments);
    }

    private static bool ShouldStartMinimizedToTray(Config config)
    {
        return config.UiItem.AutoHideStartup || config.UiItem.Hide2TrayWhenClose;
    }
